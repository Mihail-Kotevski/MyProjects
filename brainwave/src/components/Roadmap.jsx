    ...
    <Section className="overflow-hidden" id="roadmap">
      <div className="container md:pb-10">
        ...
        <div className="relative grid gap-6 md:grid-cols-2 md:gap-4 md:pb-[7rem]">
          {roadmap.map((el, i) => {
            const status = el.status === "done" ? "Done" : "In progress";
            return (
              <div
                key={i}
                className={`md:flex even:md:translate-y-[7rem] p-0.5 rounded-[2.5rem] ${
                  el.colorful ? "bg-conic-gradient" : "bg-n-6"
                }`}
              >
                <div className="relative p-8 bg-n-8 rounded-[2rem] overflow-hidden xl:p-15">
                  ...
                  <div className="relative z-1">
                    <div className="flex items-center justify-between max-w-[27rem] mb-8 md:mb-20">
                      <Tagline>{el.date}</Tagline>
                      <div className="flex items-center px-4 py-1 bg-n-1 rounded text-n-8">
                        <img
                          src={el.status === "done" ? check2 : loading1}
                          alt={status}
                          className="mr-2.5"
                          width={16}
                          height={16}
                    ...
                    <p className="body-2 text-n-4">{el.text}</p>
                  </div>
                </div>
              </div>
            );
